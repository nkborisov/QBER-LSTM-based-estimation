...
#axs[0].plot(block_indices, delta_weight, 'b', label=r'$delta_{weight}$')
# axs[0].plot(block_indices, delta_est, 'r', label=r'$delta_{est}$ (z = 1.75)')
...
#axs[0].plot(block_indices[n_beg:n_end], e_mu_est_eval[n_beg:n_end], 'fuchsia', label=r'$E_\mu$ (by decoy est)')
#axs[0].plot(block_indices[n_beg:n_end], e_nu1[n_beg:n_end], 'r', label=r'$E_{\nu_1}$')
#axs[0].plot(block_indices[n_beg:n_end], e_mu_est_by_code[n_beg:n_end], 'chartreuse', label=r'$E_\mu$ by code')
#axs[0].plot(block_indices[n_beg:n_end], rounds_count[n_beg:n_end], 'darksalmon', label=r'EC rounds count')

# axs[0].plot(block_indices, delta, 'b', label=r'$\delta E_\mu$')
axs[0].grid()
...
#axs[3].plot(block_indices[n_beg:n_end], q_mu[n_beg:n_end], 'r', label=r'$Q_\mu$')
#axs[3].plot(block_indices[n_beg:n_end], q_nu1[n_beg:n_end], 'g', label=r'$Q_{\nu1}')
#axs[3].plot(block_indices[n_beg:n_end], q_nu2[n_beg:n_end], 'b', label=r'$Q_{\nu2}$')
...
#axs[3].plot(block_indices[n_beg:n_end], delta_est[n_beg:n_end], 'darksalmon', label=r'$E_\mu$')
axs[3].plot(block_indices[n_beg:n_end], delta_ema[n_beg:n_end], 'purple', label=r'$\delta_{EMA}$')

#axs[3].plot(block_indices[n_beg:n_end], e_nu1[n_beg:n_end], 'r', label=r'$E_{\nu_1}$')
#axs[3].plot(block_indices[n_beg:n_end], e_nu2[n_beg:n_end], 'g', label=r'$E_{\nu_2}$')
#cor_enu1_emu = scipy.signal.correlate(np.array(gains_list), np.array(gains_list), mode='same', method='fft')
#cor_enu1_emu /= np.max(cor_enu1_emu)
#axs[3].plot(block_indices[n_beg:n_end], rounds_count[n_beg:n_end], 'darksalmon', label=r'EC rounds count')
#axs[3].xcorr(e_mu_act, e_nu1, usevlines=True, maxlags=10, normed=True, lw=2)
axs[3].grid()
